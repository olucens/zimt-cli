...
export function resolveAuthTemplateDir(): string {
  const possible = [
...
export async function appendUserModelToSchema(targetDir: string): Promise<void> {
...
export async function addAuthDependencies(targetDir: string): Promise<void> {
...
export async function appendAuthEnvVars(targetDir: string, _projectName: string): Promise<void> {
...
export async function wireAuthIntoAppModule(appModulePath: string): Promise<void> {
...
export interface AuthCoreOptions {
  /** If auth already exists and overwrite is not set, throws without touching files. */
  overwrite?: boolean;
  silent?: boolean;
}

/**
 * Add JWT auth, user management and RBAC to an existing project. Pure core:
 * headless, no prompts, never exits or inspects the current directory.
 * Throws on failure.
 */
export async function addAuthCore(targetDir: string, opts: AuthCoreOptions = {}): Promise<void> {
  ...
  if (fs.existsSync(authModulePath) && !opts.overwrite) {
    throw new Error(
      'Auth module already exists (src/auth/auth.module.ts). Pass overwrite: true to replace it.',
    );
  }

  const authTemplateDir = resolveAuthTemplateDir();
  ...
  const foldersToAdd = ['auth', 'user', 'crypto', 'db', 'logger'];
  ...
  if (fs.existsSync(filterSrc) && !fs.existsSync(filterDest)) {
    ...
    log('✓ Copied exceptions.filter.ts');
  }
